function parseUplink(device, payload) {
    ...
    // Store Voltage Pressure
    if (decoded.Volt != null) {
        var sensor2 = device.endpoints.byAddress("2");

        if (sensor2 != null)
            sensor2.updateVoltageSensorStatus(decoded.Volt);
    };

    // Store Pipe Pressure
    if (decoded.Pipe != null) {
        var sensor3 = device.endpoints.byAddress("3");

        if (sensor3 != null)
            sensor3.updatePressureSensorStatus(decoded.Pipe);
    };
